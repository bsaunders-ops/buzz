...
pub fn run() {
    ...
    let app = app_menu::install(builder)
        ...
        .setup(move |app| {
            ...
            let call_capture_state =
                call_capture::CallCaptureCommandState::initialize(&app_handle)?;
            app.manage(call_capture_state);
            call_capture::start_suspend_watchdog(app_handle.clone());
            #[cfg(target_os = "macos")]
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            set_window_vibrancy,
            list_call_capture_devices,
            acknowledge_call_capture_consent,
            start_call_capture,
            stop_call_capture,
            get_call_capture_state,
            accept_call_copilot_suggestion,
            #[cfg(target_os = "macos")]
    ...
    app.run(move |app_handle, event| match event {
        ...
        RunEvent::WindowEvent {
            ...
            event: WindowEvent::CloseRequested { api, .. },
            ..
        } if label == "core-call-capture" || label.starts_with("huddle-") => {
            if label == "core-call-capture" {
                api.prevent_close();
                call_capture::stop_call_capture_for_window_close(app_handle);
                return;
            }
