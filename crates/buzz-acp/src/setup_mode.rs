...
fn setup_nudge_root_kind(triggering_event: &nostr::Event, looked_up_root_kind: Option<u32>) -> u32 {
    looked_up_root_kind.unwrap_or_else(|| {
        if matches!(
            triggering_event.kind.as_u16() as u32,
...
async fn publish_setup_nudge(
    ...
    let looked_up_root_kind = if let Some(root) = thread_tags.root_event_id.as_deref() {
        let root_id = nostr::EventId::from_hex(root)
            .map_err(|e| anyhow::anyhow!("invalid root event id: {e}"))?;
        let filter = nostr::Filter::new().id(root_id);
    ...
    let root_kind = setup_nudge_root_kind(triggering_event, looked_up_root_kind);
...
mod tests {
    ...
    fn setup_nudge_prefers_looked_up_root_kind() {
        ...
        assert_eq!(
            setup_nudge_root_kind(&trigger, Some(KIND_FORUM_POST)),
            KIND_FORUM_POST
        );
        assert_eq!(setup_nudge_root_kind(&trigger, None), KIND_STREAM_MESSAGE);
        ...
        .sign_with_keys(&nostr::Keys::generate())
        .unwrap();
        assert_eq!(setup_nudge_root_kind(&forum_comment, None), KIND_FORUM_POST);
    }
